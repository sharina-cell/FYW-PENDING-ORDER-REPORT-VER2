...
def load_fyw_csv(file) -> pd.DataFrame:
    ...
    # Match tolerantly: exports have previously shifted case/whitespace, which
    # silently dropped every row when compared with a strict .isin().
    _norm = df['order_status'].astype(str).str.strip().str.upper()
    _targets = {s.strip().upper() for s in PENDING_STATUSES}
    pending = df[_norm.isin(_targets)].copy()
...
def build_report_rows(pending: pd.DataFrame, mp_sla_map: dict, shopee_tracking: dict,
    ...
    # No pending orders today (or none matched the pending statuses): return an
    # empty frame that still has the full expected schema, so every downstream
    # consumer (pivot, brand summary, Excel export) keeps working instead of
    # blowing up on a missing column.
    if not rows:
        return pd.DataFrame(columns=[
            'Brand', 'Channel', 'Nickname', 'Order ID', 'FYW SKU', 'Product Name',
            'Variant', 'Qty', 'Sold Price (MYR)', 'Order Date', 'MP SLA', 'FYW SLA',
