...
if base_df_report.empty:
    st.warning("⚠️ No pending orders found in this FYW CSV.")
    try:
        fyw_csv.seek(0)
        _raw = pd.read_csv(fyw_csv)
        if 'order_status' not in _raw.columns:
            st.error(
                "The CSV has no `order_status` column — the export format may have changed. "
                f"Columns found: {', '.join(map(str, _raw.columns))}"
            )
        else:
            counts = _raw['order_status'].value_counts()
            st.info(
                "Pending statuses the app looks for: "
                f"{', '.join(PENDING_STATUSES)}.\n\n"
                "Statuses actually present in this file are listed below — if the names differ "
                "(new wording, extra spaces, different case), that's why nothing matched."
            )
            st.dataframe(
                counts.rename_axis('order_status').reset_index(name='rows'),
                use_container_width=True, hide_index=True,
            )
    except Exception as e:
        st.error(f"Couldn't inspect the CSV for diagnostics: {e}")
    st.stop()
